...
class LIBYAFARAY_EXPORT ImageGrayOptimized final : public Image
{
	...
	private:
		virtual Type getType() const override { return Type::Gray; }
		...
		virtual int getInt(int x, int y) const override { return buffer_(x, y).getInt(); }
		...
		virtual void setInt(int x, int y, int val) override { buffer_(x, y).setInt(val); }
