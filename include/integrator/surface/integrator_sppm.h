...
class SppmIntegrator final : public MonteCarloIntegrator
{
	...
	private:
		...
		void prePass(RenderControl &render_control, RenderMonitor &render_monitor, int samples, int offset, bool adaptive) override;
		...
		GatherInfo traceGatherRay(Ray &ray, HitPoint &hp, RandomGenerator &random_generator, ColorLayers *color_layers, int ray_level, bool chromatic_enabled, float wavelength, const RayDivision &ray_division, const PixelSamplingData &pixel_sampling_data);
