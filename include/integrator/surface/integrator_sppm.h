...
class SppmIntegrator final : public MonteCarloIntegrator
{
	...
	private:
		...
		GatherInfo traceGatherRay(int thread_id, int ray_level, bool chromatic_enabled, float wavelength, const Ray &ray, HitPoint &hp, const RayDivision &ray_division, ColorLayers *color_layers, const Camera *camera, RandomGenerator *random_generator, const PixelSamplingData &pixel_sampling_data, bool lights_geometry_material_emit);
