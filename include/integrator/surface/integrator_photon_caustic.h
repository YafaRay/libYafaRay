...
class CausticPhotonIntegrator: public MonteCarloIntegrator
{
	...
	protected:
		...
		bool createCausticMap(RenderControl &render_control, RenderMonitor &render_monitor);
		void causticWorker(RenderControl &render_control, RenderMonitor &render_monitor, unsigned int &total_photons_shot, int thread_id, const Pdf1D *light_power_d_caustic, const std::vector<const Light *> &lights_caustic, int pb_step);
