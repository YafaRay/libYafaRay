...
class MonteCarloIntegrator: public TiledIntegrator
{
	...
	protected:
		...
		Rgb estimateOneDirectLight(RenderData &render_data, const SurfacePoint &sp, const Vec3 &wo, int n) const;
		/*! Does the actual light estimation on a specific light for the given surface point */
		Rgb doLightEstimation(RenderData &render_data, const Light *light, const SurfacePoint &sp, const Vec3 &wo, unsigned int loffs, ColorLayers *color_layers = nullptr) const;
		/*! Does recursive mc raytracing with MIS (Multiple Importance Sampling) for a given surface point */
		void recursiveRaytrace(RenderData &render_data, const Ray &ray, const BsdfFlags &bsdfs, SurfacePoint &sp, const Vec3 &wo, Rgb &col, float &alpha, int additional_depth, ColorLayers *color_layers) const;
		...
		Rgb glossy(RenderData &render_data, float &alpha, const Ray &ray, const SpDifferentials &sp_differentials, const Material *material, const BsdfFlags &mat_bsdfs, const BsdfFlags &bsdfs, const Vec3 &wo, float ray_min_dist, int additional_depth, ColorLayers *color_layers) const;
		Rgb specularReflect(RenderData &render_data, float &alpha, const Ray &ray, const SpDifferentials &sp_differentials, const Material *material, const BsdfFlags &bsdfs, const DirectionColor *reflect_data, float ray_min_dist, int additional_depth, ColorLayers *color_layers) const;
		Rgb specularRefract(RenderData &render_data, float &alpha, const Ray &ray, const SpDifferentials &sp_differentials, const Material *material, const BsdfFlags &bsdfs, const DirectionColor *refract_data, float ray_min_dist, int additional_depth, ColorLayers *color_layers) const;
