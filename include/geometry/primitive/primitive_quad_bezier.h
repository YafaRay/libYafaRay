...
class QuadBezierPrimitive : public FacePrimitive
{
	...
	private:
		...
		float getDistToNearestEdge(const Uv &uv, const Vec3 &dp_du_abs, const Vec3 &dp_dv_abs) const override { return ShapeQuad::getDistToNearestEdge(uv, dp_du_abs, dp_dv_abs); }
...
inline std::pair<Point3, Vec3> QuadBezierPrimitive::sample(const Uv &uv, float time) const
{
...
inline std::pair<Point3, Vec3> QuadBezierPrimitive::sample(const Uv &uv, const Matrix4 &obj_to_world, float time) const
{
...
inline Vec3 QuadBezierPrimitive::getGeometricNormal(const Uv &uv, float time) const
{
	return getShapeQuadAtTime(time).calculateFaceNormal();
}

inline Vec3 QuadBezierPrimitive::getGeometricNormal(const Matrix4 &obj_to_world, const Uv &uv, float time) const
{
