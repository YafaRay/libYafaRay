...
class QuadBezierPrimitive : public FacePrimitive
{
	...
	private:
		...
		template<typename T=bool> std::unique_ptr<const SurfacePoint> getSurfaceQuadBezier(const RayDifferentials *ray_differentials, const Point3f &hit_point, float time, const Uv<float> &intersect_uv, const Camera *camera, const T &obj_to_world = {}) const;
		...
		template <typename T=bool> std::array<Point3f, 4> getVerticesAsArray(const std::array<float, 3> &bezier_factors, const T &obj_to_world = {}) const;
...
template <typename T>
inline std::array<Point3f, 4> QuadBezierPrimitive::getVerticesAsArray(int time_step, const T &obj_to_world) const
{
...
template <typename T>
inline std::array<Point3f, 4> QuadBezierPrimitive::getVerticesAsArray(const std::array<float, 3> &bezier_factors, const T &obj_to_world) const
{
...
inline std::array<Point3f, 4> QuadBezierPrimitive::getOrcoVertices(int time_step) const
{
	return { getOrcoVertex(0, time_step), getOrcoVertex(1, time_step), getOrcoVertex(2, time_step), getOrcoVertex(3, time_step) };
}

template <typename T>
inline std::array<Vec3f, 4> QuadBezierPrimitive::getVerticesNormals(int time_step, const Vec3f &surface_normal_world, const T &obj_to_world) const
{
...
inline std::pair<float, Uv<float>> QuadBezierPrimitive::intersect(const Point3f &from, const Vec3f &dir, float time) const
{
	return getShapeAtTime(time).intersect(from, dir);
}

inline std::pair<float, Uv<float>> QuadBezierPrimitive::intersect(const Point3f &from, const Vec3f &dir, float time, const Matrix4f &obj_to_world) const
{
...
inline float QuadBezierPrimitive::surfaceArea(float time, const Matrix4f &obj_to_world) const
{
	return getShapeAtTime(time, obj_to_world).surfaceArea();
}

inline std::pair<Point3f, Vec3f> QuadBezierPrimitive::sample(const Uv<float> &uv, float time) const
{
...
inline std::pair<Point3f, Vec3f> QuadBezierPrimitive::sample(const Uv<float> &uv, float time, const Matrix4f &obj_to_world) const
{
...
inline Vec3f QuadBezierPrimitive::getGeometricNormal(const Uv<float> &uv, float time, bool) const
{
	return getShapeAtTime(time).calculateFaceNormal();
}

inline Vec3f QuadBezierPrimitive::getGeometricNormal(const Uv<float> &uv, float time, const Matrix4f &obj_to_world) const
{
	const Vec3f normal {getShapeAtTime(time, obj_to_world).calculateFaceNormal()};
...
inline Bound QuadBezierPrimitive::getBound(const Matrix4f &obj_to_world) const
{
