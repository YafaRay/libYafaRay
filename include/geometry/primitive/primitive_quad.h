...
class QuadPrimitive : public FacePrimitive
{
	...
	private:
		...
		template<typename T=bool> std::unique_ptr<const SurfacePoint> getSurfaceQuad(const RayDifferentials *ray_differentials, const Point3f &hit_point, float time, const Uv<float> &intersect_uv, const Camera *camera, const T &obj_to_world = {}) const;
		...
		float getDistToNearestEdge(const Uv<float> &uv, const Uv<Vec3f> &dp_abs) const override { return ShapeQuad::getDistToNearestEdge(uv, dp_abs); }
		Vec3f getGeometricNormal(const Matrix4f &obj_to_world) const;
		Vec3f getGeometricNormal(bool = false) const;
		template <typename T=bool> std::array<Point3f, 4> getVerticesAsArray(int time_step, const T &obj_to_world = {}) const;
		inline std::array<Point3f, 4> getOrcoVertices(int time_step) const;
		template <typename T=bool> std::array<Vec3f, 4> getVerticesNormals(int time_step, const Vec3f &surface_normal_world, const T &obj_to_world = {}) const;
		inline std::array<Uv<float>, 4> getVerticesUvs() const;
...
inline QuadPrimitive::QuadPrimitive(std::vector<int> &&vertices_indices, std::vector<int> &&vertices_uv_indices, const MeshObject &mesh_object) : FacePrimitive{std::move(vertices_indices), std::move(vertices_uv_indices), mesh_object},
	face_normal_geometric_{ ShapeQuad{getVerticesAsArray(0)}.calculateFaceNormal()}
{
}

template <typename T>
inline std::array<Point3f, 4> QuadPrimitive::getVerticesAsArray(int time_step, const T &obj_to_world) const
{
...
inline std::array<Point3f, 4> QuadPrimitive::getOrcoVertices(int time_step) const
{
	return { getOrcoVertex(0, time_step), getOrcoVertex(1, time_step), getOrcoVertex(2, time_step), getOrcoVertex(3, time_step) };
}

template <typename T>
inline std::array<Vec3f, 4> QuadPrimitive::getVerticesNormals(int time_step, const Vec3f &surface_normal_world, const T &obj_to_world) const
{
...
inline std::array<Uv<float>, 4> QuadPrimitive::getVerticesUvs() const
{
	return { getVertexUv(0), getVertexUv(1), getVertexUv(2), getVertexUv(3) };
}

inline std::pair<float, Uv<float>> QuadPrimitive::intersect(const Point3f &from, const Vec3f &dir, float time) const
{
	return ShapeQuad{getVerticesAsArray(0)}.intersect(from, dir);
}

inline std::pair<float, Uv<float>> QuadPrimitive::intersect(const Point3f &from, const Vec3f &dir, float time, const Matrix4f &obj_to_world) const
{
	return ShapeQuad{getVerticesAsArray(0, obj_to_world)}.intersect(from, dir);
}

inline float QuadPrimitive::surfaceArea(float time) const
{
	return ShapeQuad{getVerticesAsArray(0)}.surfaceArea();
}

inline float QuadPrimitive::surfaceArea(float time, const Matrix4f &obj_to_world) const
{
	return ShapeQuad{getVerticesAsArray(0, obj_to_world)}.surfaceArea();
}

inline std::pair<Point3f, Vec3f> QuadPrimitive::sample(const Uv<float> &uv, float time) const
{
	return {ShapeQuad{getVerticesAsArray(0)}.sample(uv), getGeometricNormal()};
}

inline std::pair<Point3f, Vec3f> QuadPrimitive::sample(const Uv<float> &uv, float time, const Matrix4f &obj_to_world) const
{
	return {ShapeQuad{getVerticesAsArray(0, obj_to_world)}.sample(uv), getGeometricNormal(obj_to_world) };
}
...
inline Bound QuadPrimitive::getBound(const Matrix4f &obj_to_world) const
{
	return FacePrimitive::getBound(getVerticesAsVector(0, obj_to_world));
}

inline Vec3f QuadPrimitive::getGeometricNormal(bool) const
{
	return face_normal_geometric_;
}

inline Vec3f QuadPrimitive::getGeometricNormal(const Uv<float> &, float, bool) const
{
	return getGeometricNormal();
}

inline Vec3f QuadPrimitive::getGeometricNormal(const Uv<float> &, float, const Matrix4f &obj_to_world) const
{
	return getGeometricNormal(obj_to_world);
}

inline Vec3f QuadPrimitive::getGeometricNormal(const Matrix4f &obj_to_world) const
{
