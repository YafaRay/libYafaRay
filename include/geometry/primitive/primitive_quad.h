...
inline std::pair<Point3, Vec3> QuadPrimitive::sample(const Uv &uv, float time) const
{
	return {
			ShapeQuad{{
							  getVertex(0, 0),
...
inline std::pair<Point3, Vec3> QuadPrimitive::sample(const Uv &uv, const Matrix4 &obj_to_world, float time) const
{
	return {
			ShapeQuad{{
							  getVertex(0, obj_to_world, 0),
...
inline Vec3 QuadPrimitive::getGeometricNormal(const Uv &, float) const
{
	return getGeometricNormal();
}

inline Vec3 QuadPrimitive::getGeometricNormal(const Matrix4 &obj_to_world, const Uv &, float) const
{
