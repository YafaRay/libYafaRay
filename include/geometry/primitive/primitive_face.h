...
{
	if(obj_to_world) return (*obj_to_world) * getVertex(vertex_number, time_step);
...
inline Vec3 FacePrimitive::getVertexNormal(size_t vertex_number, const Vec3 &surface_normal_world, const Matrix4 *obj_to_world, size_t time_step) const
{
	...
	{
		if(obj_to_world) return ((*obj_to_world) * getVertexNormal(vertex_number, surface_normal_world, time_step)).normalize();
		else return getVertexNormal(vertex_number, surface_normal_world, time_step);
	}
	else return surface_normal_world;
}
