...
{
	public:
		static std::unique_ptr<Material> factory(ParamMap &params, std::list< ParamMap > &eparans, Scene &scene);
	...
	private:
		...
		virtual Rgb emit(const RenderData &render_data, const SurfacePoint &sp, const Vec3 &wo) const;
		virtual float pdf(const RenderData &render_data, const SurfacePoint &sp, const Vec3 &wo, const Vec3 &wi, const BsdfFlags &bsdfs) const;
