...
Rgb MonteCarloIntegrator::estimateAllDirectLight(RandomGenerator &random_generator, ColorLayers *color_layers, bool chromatic_enabled, float wavelength, const SurfacePoint &sp, const Vec3f &wo, const RayDivision &ray_division, const PixelSamplingData &pixel_sampling_data) const
{
	Rgb col{0.f};
...
Rgb MonteCarloIntegrator::diracLight(RandomGenerator &random_generator, ColorLayers *color_layers, const Light *light, const Vec3f &wo, const SurfacePoint &sp, bool cast_shadows, float time) const
{
	if(auto[hit, light_ray, lcol]{light->illuminate(sp.p_, time)}; hit)
	{
		...
		if(color_layers)
		{
			if(flags::have(color_layers->getFlags(), LayerDef::Flags::DiffuseLayers))
			{
		...
		if(!shadowed || color_layer_diffuse_no_shadow)
		{
			...
			if(transparent_shadows_ && cast_shadows) lcol *= scol;
			if(color_layers)
			{
		...
		if(color_layers)
		{
			if(shadowed && flags::have(color_layers->getFlags(), LayerDef::Flags::IndexLayers) && shadow_casting_primitive)
			{
...
Rgb MonteCarloIntegrator::areaLightSampleLight(Halton &hal_2, Halton &hal_3, RandomGenerator &random_generator, ColorLayers *color_layers, const Light *light, const Vec3f &wo, const SurfacePoint &sp, bool cast_shadows, unsigned int num_samples, float inv_num_samples, float time) const
{
	Rgb col{0.f};
	...
	for(unsigned int i = 0; i < num_samples; ++i)
	{
		...
		if(auto[hit, light_ray]{light->illumSample(sp.p_, ls, time)}; hit)
		{
			...
			if((!shadowed && ls.pdf_ > 1e-6f) || layer_diffuse_no_shadow)
			{
				...
				if(transparent_shadows_ && cast_shadows) ls.col_ *= scol;
				if(vol_integrator_)
				{
			...
			if(color_layers && (shadowed || ls.pdf_ <= 1e-6f) && flags::have(color_layers->getFlags(), LayerDef::Flags::IndexLayers) && shadow_casting_primitive)
			{
				float mask_obj_index = 0.f, mask_mat_index = 0.f;
				mask_obj_index = shadow_casting_primitive->getObjectIndex();    //Object index of the object casting the shadow
				if(const Material *casting_material = shadow_casting_primitive->getMaterial()) mask_mat_index = casting_material->getIndex();    //Material index of the object casting the shadow
...
Rgb MonteCarloIntegrator::areaLightSampleMaterial(Halton &hal_2, Halton &hal_3, RandomGenerator &random_generator, ColorLayers *color_layers, bool chromatic_enabled, float wavelength, const Light *light, const Vec3f &wo, const SurfacePoint &sp, bool cast_shadows, unsigned int num_samples, float inv_num_samples) const
{
				...
				if((!shadowed && light_pdf > 1e-6f) || layer_diffuse_no_shadow)
				{
					...
					if(color_layers)
					{
						if(layer_diffuse || layer_diffuse_no_shadow)
						{
							...
							if(layer_diffuse && !shadowed && light_pdf > 1e-6f && flags::have(s.sampled_flags_, BsdfFlags::Diffuse)) layer_diffuse->accum_ += tmp_col;
						}
						if(layer_glossy)
						{
							const Rgb tmp_col = sp.sample(wo, b_ray.dir_, s, W, chromatic_enabled, wavelength, camera_) * lcol * w * W;
							if(!shadowed && light_pdf > 1e-6f && flags::have(s.sampled_flags_, BsdfFlags::Glossy)) layer_glossy->accum_ += tmp_col;
						}
					}
...
Rgb MonteCarloIntegrator::doLightEstimation(RandomGenerator &random_generator, ColorLayers *color_layers, bool chromatic_enabled, float wavelength, const Light *light, const SurfacePoint &sp, const Vec3f &wo, unsigned int loffs, const RayDivision &ray_division, const PixelSamplingData &pixel_sampling_data) const
{
	Rgb col{0.f};
...
Rgb MonteCarloIntegrator::causticPhotons(ColorLayers *color_layers, const Ray<float> &ray, const SurfacePoint &sp, const Vec3f &wo, float clamp_indirect, const PhotonMap *caustic_map, float caustic_radius, int n_caus_search)
{
...
void MonteCarloIntegrator::causticWorker(FastRandom &fast_random, unsigned int &total_photons_shot, int thread_id, const Pdf1D *light_power_d_caustic, const std::vector<const Light *> &lights_caustic, int pb_step)
{
	...
	while(!done)
	{
		...
		while(true)
		{
			...
			if(flags::have(mat_bsdfs, (BsdfFlags::Diffuse | BsdfFlags::Glossy)))
			{
...
Rgb MonteCarloIntegrator::estimateCausticPhotons(const SurfacePoint &sp, const Vec3f &wo, const PhotonMap *caustic_map, float caustic_radius, int n_caus_search)
{
...
std::pair<Rgb, float> MonteCarloIntegrator::dispersive(FastRandom &fast_random, RandomGenerator &random_generator, std::vector<int> &correlative_sample_number, ColorLayers *color_layers, int thread_id, int ray_level, bool chromatic_enabled, const SurfacePoint &sp, const Material *material, BsdfFlags bsdfs, const Vec3f &wo, int additional_depth, const RayDivision &ray_division, const PixelSamplingData &pixel_sampling_data) const
{
	...
	std::unique_ptr<Ray<float>> ref_ray_chromatic_volume; //Reference ray used for chromatic/dispersive volume color calculation only. FIXME: it only uses one of the sampled reference rays for volume calculations, not sure if this is ok??
	for(int ns = 0; ns < ray_samples_dispersive; ++ns)
	{
		...
		if(s.pdf_ > 1.0e-6f && flags::have(s.sampled_flags_, BsdfFlags::Dispersive))
		{
			...
			Ray<float> ref_ray{sp.p_, wi, sp.time_, ray_min_dist_};
			...
			if(!ref_ray_chromatic_volume) ref_ray_chromatic_volume = std::make_unique<Ray<float>>(ref_ray, Ray<float>::DifferentialsCopy::No);
		}
	}
	if(ref_ray_chromatic_volume && flags::have(bsdfs, BsdfFlags::Volumetric))
	{
...
std::pair<Rgb, float> MonteCarloIntegrator::glossy(FastRandom &fast_random, RandomGenerator &random_generator, std::vector<int> &correlative_sample_number, ColorLayers *color_layers, int thread_id, int ray_level, bool chromatic_enabled, float wavelength, const Ray<float> &ray, const SurfacePoint &sp, BsdfFlags bsdfs, const Vec3f &wo, int additional_depth, const RayDivision &ray_division, const PixelSamplingData &pixel_sampling_data) const
{
	if(!flags::have(sp.mat_data_->bsdf_flags_, BsdfFlags::Reflect)) return {Rgb{0.f}, -1.f}; //If alpha is -1.f we consider the result invalid and do not use it to accumulate alpha
	...
	for(int ns = 0; ns < ray_samples_glossy; ++ns)
	{
		...
		const float s_1 = hal_2.getNext();
		const float s_2 = hal_3.getNext();
		if(flags::have(sp.mat_data_->bsdf_flags_, BsdfFlags::Transmit))
		{
			...
			if(!flags::have(s.sampled_flags_, BsdfFlags::Dispersive))
			{
				const auto [reflect_col, reflect_alpha] = glossyReflect(fast_random, random_generator, correlative_sample_number, thread_id, ray_level, chromatic_enabled, wavelength, ray, sp, bsdfs, additional_depth, pixel_sampling_data, ray_division_new, mcol[0], w[0], dir[0]);
				gcol += reflect_col;
			...
			{
				const auto [transmit_col, transmit_alpha] = glossyTransmit(fast_random, random_generator, correlative_sample_number, thread_id, ray_level, chromatic_enabled, wavelength, ray, sp, bsdfs, additional_depth, pixel_sampling_data, ray_division_new, mcol[1], w[1], dir[1]);
				gcol += transmit_col;
		...
		{
			const auto [reflect_col, reflect_alpha] = glossyReflectNoTransmit(fast_random, random_generator, correlative_sample_number, thread_id, ray_level, chromatic_enabled, wavelength, ray, sp, bsdfs, wo, additional_depth, pixel_sampling_data, ray_division_new, s_1, s_2);
			gcol += reflect_col;
...
std::pair<Rgb, float> MonteCarloIntegrator::glossyReflect(FastRandom &fast_random, RandomGenerator &random_generator, std::vector<int> &correlative_sample_number, int thread_id, int ray_level, bool chromatic_enabled, float wavelength, const Ray<float> &ray, const SurfacePoint &sp, BsdfFlags bsdfs, int additional_depth, const PixelSamplingData &pixel_sampling_data, const RayDivision &ray_division_new, const Rgb &reflect_color, float w, const Vec3f &dir) const
{
	Ray<float> ref_ray{sp.p_, dir, ray.time_, ray_min_dist_};
	...
	if(flags::have(bsdfs, BsdfFlags::Volumetric))
	{
...
std::pair<Rgb, float> MonteCarloIntegrator::glossyTransmit(FastRandom &fast_random, RandomGenerator &random_generator, std::vector<int> &correlative_sample_number, int thread_id, int ray_level, bool chromatic_enabled, float wavelength, const Ray<float> &ray, const SurfacePoint &sp, BsdfFlags bsdfs, int additional_depth, const PixelSamplingData &pixel_sampling_data, const RayDivision &ray_division_new, const Rgb &transmit_col, float w, const Vec3f &dir) const
{
	Ray<float> ref_ray{sp.p_, dir, ray.time_, ray_min_dist_};
...
std::pair<Rgb, float> MonteCarloIntegrator::glossyReflectNoTransmit(FastRandom &fast_random, RandomGenerator &random_generator, std::vector<int> &correlative_sample_number, int thread_id, int ray_level, bool chromatic_enabled, float wavelength, const Ray<float> &ray, const SurfacePoint &sp, BsdfFlags bsdfs, const Vec3f &wo, int additional_depth, const PixelSamplingData &pixel_sampling_data, const RayDivision &ray_division_new, const float s_1, const float s_2) const
{
	float w = 0.f;
	...
	Ray<float> ref_ray{sp.p_, wi, ray.time_, ray_min_dist_};
...
std::pair<Rgb, float> MonteCarloIntegrator::specularReflect(FastRandom &fast_random, RandomGenerator &random_generator, std::vector<int> &correlative_sample_number, ColorLayers *color_layers, int thread_id, int ray_level, bool chromatic_enabled, float wavelength, const Ray<float> &ray, const SurfacePoint &sp, const Material *material, BsdfFlags bsdfs, const DirectionColor *reflect_data, int additional_depth, const RayDivision &ray_division, const PixelSamplingData &pixel_sampling_data) const
{
	Ray<float> ref_ray{sp.p_, reflect_data->dir_, ray.time_, ray_min_dist_};
	if(ray.differentials_) ref_ray.differentials_ = sp.reflectedRay(ray.differentials_.get(), ray.dir_, ref_ray.dir_);
...
std::pair<Rgb, float> MonteCarloIntegrator::specularRefract(FastRandom &fast_random, RandomGenerator &random_generator, std::vector<int> &correlative_sample_number, ColorLayers *color_layers, int thread_id, int ray_level, bool chromatic_enabled, float wavelength, const Ray<float> &ray, const SurfacePoint &sp, const Material *material, BsdfFlags bsdfs, const DirectionColor *refract_data, int additional_depth, const RayDivision &ray_division, const PixelSamplingData &pixel_sampling_data) const
{
	Ray<float> ref_ray;
...
std::pair<Rgb, float> MonteCarloIntegrator::recursiveRaytrace(FastRandom &fast_random, RandomGenerator &random_generator, std::vector<int> &correlative_sample_number, ColorLayers *color_layers, int thread_id, int ray_level, bool chromatic_enabled, float wavelength, const Ray<float> &ray, BsdfFlags bsdfs, const SurfacePoint &sp, const Vec3f &wo, int additional_depth, const RayDivision &ray_division, const PixelSamplingData &pixel_sampling_data) const
{
	Rgb col {0.f};
	float alpha = 0.f;
	int alpha_count = 0;
	if(ray_level <= (r_depth_ + additional_depth))
	{
		...
		if(flags::have(bsdfs, BsdfFlags::Dispersive) && chromatic_enabled)
		{
			const auto [dispersive_col, dispersive_alpha] = dispersive(fast_random, random_generator, correlative_sample_number, color_layers, thread_id, ray_level, chromatic_enabled, sp, sp.getMaterial(), bsdfs, wo, additional_depth, ray_division, pixel_sampling_data);
			col += dispersive_col;
			alpha += dispersive_alpha;
			++alpha_count;
		}
		if(ray_level < 20 && flags::have(bsdfs, BsdfFlags::Glossy | BsdfFlags::Specular | BsdfFlags::Filter))
		{
			// glossy reflection with recursive raytracing:
			if(flags::have(bsdfs, BsdfFlags::Glossy))
			{
				const auto [glossy_col, glossy_alpha] = glossy(fast_random, random_generator, correlative_sample_number, color_layers, thread_id, ray_level, chromatic_enabled, wavelength, ray, sp, bsdfs, wo, additional_depth, ray_division, pixel_sampling_data);
			...
			if(flags::have(bsdfs, (BsdfFlags::Specular | BsdfFlags::Filter)))
			{
				const Specular specular = sp.getSpecular(ray_level, wo, chromatic_enabled, wavelength);
				if(specular.reflect_)
				{
					const auto [specular_col, specular_alpha] = specularReflect(fast_random, random_generator, correlative_sample_number, color_layers, thread_id, ray_level, chromatic_enabled, wavelength, ray, sp, sp.getMaterial(), bsdfs, specular.reflect_.get(), additional_depth, ray_division, pixel_sampling_data);
					col += specular_col;
				...
				{
					const auto [specular_col, specular_alpha] = specularRefract(fast_random, random_generator, correlative_sample_number, color_layers, thread_id, ray_level, chromatic_enabled, wavelength, ray, sp, sp.getMaterial(), bsdfs, specular.refract_.get(), additional_depth, ray_division, pixel_sampling_data);
					col += specular_col;
					alpha += specular_alpha;
					++alpha_count;
				}
