...
ExportC::ExportC(const char *fname, const ::yafaray_LoggerCallback_t logger_callback, void *callback_data, ::yafaray_DisplayConsole_t logger_display_console) : Interface(logger_callback, callback_data, logger_display_console), file_name_(std::string(fname))
{
	...
	file_ << "/* ANSI C89/C90 file generated by libYafaRay C Export */\n";
	file_ << "/* To build use your favorite ANSI C compiler/linker, pointing to libYafaRay include/library files */\n";
	file_ << "/* For example in Linux using GCC */\n";
	file_ << "/* LD_LIBRARY_PATH=(path to folder with libyafaray libs) gcc -o libyafaray_example_executable -ansi -I(path to folder with libyafaray includes) -L(path to folder with libyafaray libs) (yafaray_c_file_name.c) -lyafaray4 */\n";
	file_ << "/* To run the executable */\n";
	file_ << "/* LD_LIBRARY_PATH=(path to folder with libyafaray libs) ./libyafaray_example_executable */\n\n";
	file_ << "#include <yafaray_c_api.h>\n";
	file_ << "#include <stddef.h>\n\n";
	file_ << "int main()\n";
	file_ << "{\n";
	file_ << "\t" << "yafaray_Interface_t *yi = yafaray_createInterface(YAFARAY_INTERFACE_FOR_RENDERING, NULL, NULL, NULL, YAFARAY_DISPLAY_CONSOLE_NORMAL);\n";
	file_ << "\t" << "yafaray_setConsoleLogColorsEnabled(yi, YAFARAY_BOOL_TRUE);\n";
	file_ << "\t" << "yafaray_setConsoleVerbosityLevel(yi, YAFARAY_LOG_LEVEL_INFO);\n\n";
}
...
void ExportC::render(std::shared_ptr<ProgressBar> progress_bar) noexcept
{
	...
	file_ << "\t" << "yafaray_render(yi, NULL, NULL, YAFARAY_DISPLAY_CONSOLE_NORMAL);\n";
	file_ << "\t" << "yafaray_destroyInterface(yi);\n";
	file_ << "\t" << "return 0;\n";
	file_ << "}\n";
	params_->clear();
	eparams_->clear();
