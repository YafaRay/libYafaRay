...
std::unique_ptr<const SurfacePoint> TriangleBezierPrimitive::getSurface(const RayDifferentials *ray_differentials, const Point3f &hit_point, float time, const Uv<float> &intersect_uv, const Camera *camera) const
{
	return getSurfaceTriangleBezier(ray_differentials, hit_point, time, intersect_uv, camera);
}

std::unique_ptr<const SurfacePoint> TriangleBezierPrimitive::getSurface(const RayDifferentials *ray_differentials, const Point3f &hit_point, float time, const Uv<float> &intersect_uv, const Camera *camera, const Matrix4f &obj_to_world) const
{
	return getSurfaceTriangleBezier(ray_differentials, hit_point, time, intersect_uv, camera, obj_to_world);
}

template<typename T>
std::unique_ptr<const SurfacePoint> TriangleBezierPrimitive::getSurfaceTriangleBezier(const RayDifferentials *ray_differentials, const Point3f &hit_point, float time, const Uv<float> &intersect_uv, const Camera *camera, const T &obj_to_world) const
{
