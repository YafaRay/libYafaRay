...
std::pair<Texture *, ParamError> DistortedNoiseTexture::factory(Logger &logger, const Scene &scene, const std::string &name, const ParamMap &param_map)
{
	auto param_error{Params::meta_.check(param_map, {"type"}, {"ramp_item_"})};
	auto result {new DistortedNoiseTexture(logger, param_error, param_map)};
	if(param_error.flags_ != ParamError::Flags::Ok) logger.logWarning(param_error.print<DistortedNoiseTexture>(name, {"type"}));
	return {result, param_error};
}

DistortedNoiseTexture::DistortedNoiseTexture(Logger &logger, ParamError &param_error, const ParamMap &param_map) : Texture{logger, param_error, param_map}, params_{param_error, param_map}
{
