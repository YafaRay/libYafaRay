...
std::pair<std::unique_ptr<Material>, ParamError> GlossyMaterial::factory(Logger &logger, const Scene &scene, const std::string &name, const ParamMap &param_map, const std::list<ParamMap> &nodes_param_maps, size_t id)
{
...
GlossyMaterial::GlossyMaterial(Logger &logger, ParamError &param_error, const ParamMap &param_map, size_t id) :
