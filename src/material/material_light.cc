...
std::pair<Material *, ParamError> LightMaterial::factory(Logger &logger, const Scene &scene, const std::string &name, const ParamMap &param_map, const std::list<ParamMap> &nodes_param_maps)
{
	auto param_error{Params::meta_.check(param_map, {"type"}, {})};
	auto mat = new LightMaterial(logger, param_error, param_map);
	if(param_error.flags_ != ParamError::Flags::Ok) logger.logWarning(param_error.print<LightMaterial>(name, {"type"}));
	return {mat, param_error};
}

LightMaterial::LightMaterial(Logger &logger, ParamError &param_error, const ParamMap &param_map) :
