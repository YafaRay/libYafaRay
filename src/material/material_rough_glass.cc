...
ParamMap RoughGlassMaterial::Params::getAsParamMap(bool only_non_default) const
{
	PARAM_SAVE_START;
	PARAM_SAVE(ior_);
...
std::pair<std::unique_ptr<Material>, ParamResult> RoughGlassMaterial::factory(Logger &logger, const Scene &scene, const std::string &name, const ParamMap &param_map, const std::list<ParamMap> &nodes_param_maps)
{
	...
	if(param_result.notOk()) logger.logWarning(param_result.print<RoughGlassMaterial>(name, {"type"}));
	return {std::move(material), param_result};
}

RoughGlassMaterial::RoughGlassMaterial(Logger &logger, ParamResult &param_result, const ParamMap &param_map, const SceneItems <Material> &materials) :
