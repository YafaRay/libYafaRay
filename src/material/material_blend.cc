...
std::map<std::string, const ParamMeta *> BlendMaterial::Params::getParamMetaMap()
{
	auto param_meta_map{ParentClassType_t::Params::getParamMetaMap()};
	PARAM_META(material_1_name_);
	PARAM_META(material_2_name_);
	PARAM_META(blend_value_);
...
std::pair<std::unique_ptr<Material>, ParamResult> BlendMaterial::factory(Logger &logger, const Scene &scene, const std::string &name, const ParamMap &param_map, const std::list<ParamMap> &nodes_param_maps)
{
	...
	auto material{std::make_unique<BlendMaterial>(logger, param_result, param_map, mat_1_id, mat_2_id, scene.getMaterials())};
