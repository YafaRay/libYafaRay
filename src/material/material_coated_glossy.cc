...
std::map<std::string, const ParamMeta *> CoatedGlossyMaterial::Params::getParamMetaMap()
{
	auto param_meta_map{ParentClassType_t::Params::getParamMetaMap()};
	PARAM_META(glossy_color_);
	PARAM_META(diffuse_color_);
	PARAM_META(diffuse_reflect_);
	PARAM_META(glossy_reflect_);
	PARAM_META(as_diffuse_);
	PARAM_META(exponent_);
	PARAM_META(anisotropic_);
	PARAM_META(ior_);
...
std::pair<std::unique_ptr<Material>, ParamResult> CoatedGlossyMaterial::factory(Logger &logger, const Scene &scene, const std::string &name, const ParamMap &param_map, const std::list<ParamMap> &nodes_param_maps)
{
	...
	auto material{std::make_unique<CoatedGlossyMaterial>(logger, param_result, param_map, scene.getMaterials())};
