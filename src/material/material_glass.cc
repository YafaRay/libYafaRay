...
std::pair<std::unique_ptr<Material>, ParamError> GlassMaterial::factory(Logger &logger, const Scene &scene, const std::string &name, const ParamMap &param_map, const std::list<ParamMap> &nodes_param_maps, size_t id)
{
...
GlassMaterial::GlassMaterial(Logger &logger, ParamError &param_error, const ParamMap &param_map, size_t id) :
