...
std::map<std::string, const ParamMeta *> GlassMaterial::Params::getParamMetaMap()
{
...
GlassMaterial::Params::Params(ParamResult &param_result, const ParamMap &param_map)
{
	PARAM_LOAD(ior_);
...
std::pair<std::unique_ptr<Material>, ParamResult> GlassMaterial::factory(Logger &logger, const Scene &scene, const std::string &name, const ParamMap &param_map, const std::list<ParamMap> &nodes_param_maps)
{
	auto param_result{class_meta::check<Params>(param_map, {"type"}, {})};
	param_result.merge(checkShadersParams<Params, ShaderNodeType>(param_map));
	auto material{std::make_unique<GlassMaterial>(logger, param_result, param_map, scene.getMaterials())};
...
GlassMaterial::GlassMaterial(Logger &logger, ParamResult &param_result, const ParamMap &param_map, const Items <Material> &materials) :
